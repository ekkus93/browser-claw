...
- [x] Append audit events for: <!-- all sub-items wired (runtime load/fail/fallback, provider test, LLM request, effect lifecycle, tool lifecycle, memory CRUD+retrieval, skill lifecycle, secret lock/unlock/delete, backup export/import, model download/load/delete) + no-raw-secrets guard. -->
  ...
  - [x] effect emitted/resolved/failed; <!-- Opt-in verbose runtime audit (default off so the log isn't flooded). The "tied to effect-port contract (Phase 1.2, blocked)" note was STALE — every Effect already carries a stable id and RuntimeHost.submit() sees both emitted effects (dispatch output) and resolutions (resolve_effect commands), so this is a pure TS host change, no claw-schema/WASM rebuild. RuntimeHost emits runtime.effect_emitted (per non-terminal effect; audit_append + runtime_snapshot_save excluded as terminal/noise) and runtime.effect_resolved / runtime.effect_failed (from the resolve_effect result.ok). Gated by the verboseRuntimeAudit setting (cached in runtimeAuditPref.ts, toggled in Settings > Developer, loaded at boot). Infra-level effect failures still audit via effectExecutor.failEffect (unchanged). Tested: runtimeHost.test.ts ("records ... lifecycle audits only when verbose is on" + "records no lifecycle audits when verbose is off") + runtimeAuditPref.test.ts. -->
  ...
  - [x] secret unlocked/locked/deleted; <!-- The vault emits secret_unlocked (unlock/setup), secret_locked (lock/auto-lock), secret_unlock_failed (status:failure), and now secret_deleted (removeSecret — names the label, never the plaintext). These are now DURABLE: createReduxVaultObserver, when given the db, routes vault audits through recordAudit (db.audit_events + live tail) instead of the live-tail-only dispatch; vault.ts wires the db. SecurityScreen's existing Delete button drives removeSecret. Tested: secretVault.test "emits a secret_deleted audit ... on removeSecret" + the existing lock/unlock test; vaultWiring.test "persists vault audit events durably when given a db (real secret_deleted)" asserts the row lands in db.audit_events with status success. -->
...
- [x] Save snapshot: <!-- all triggers covered: after user message, after each effect emitted/resolved (debounced per-turn + per-resolve), after idle (= drained pending), after error (flush-on-fatal), before unload. Literal per-effect saves and a deterministic idle/error status enum were deliberately omitted (IndexedDB thrash / no consumer). -->
  - [x] after user message accepted; <!-- RuntimeHost.submit() schedules a coalesced save after the turn settles (runtimeHost.ts) -->
  - [x] after each effect emitted; <!-- COVERED (not literal-per-effect, by design). The prior "contract-blocked" note was stale: the runtime already exposes a stable snapshot via port.snapshot() and the host sees every effect, so no contract change was needed. RuntimeHost.submit() schedules a coalesced (debounced) save after draining the turn's emitted effects; effects within a turn are in memory and the post-turn save captures the resulting state. Literal per-emitted-effect saves were deliberately NOT done — they would thrash IndexedDB with no durability gain. -->
  - [x] after each effect resolved; <!-- COVERED: every resolve_effect is its own RuntimeHost.submit() call (ports call host.submit to resolve), which schedules a debounced save — so state IS persisted after each resolution, coalesced. -->
  - [x] after runtime enters idle; <!-- COVERED. "Idle" = the pending-effects map is empty (no effect awaiting resolution), derivable any time from port.snapshot().pending. When the last effect resolves, that resolve is its own RuntimeHost.submit() which schedules a debounced save of the now-drained (idle) state — so the idle state IS persisted. A separate explicit idle flag/state would be redundant; an idle status enum inside the deterministic runtime was deliberately NOT added (no consumer; the runtime never executes effects so it gains nothing from tracking it). -->
  - [x] after runtime enters error; <!-- RuntimeHost.submit() now wraps the effect loop in try/catch: a fatal effect failure (failEffect throws) flushes a snapshot (#flushOnError -> saveSnapshot, best-effort) before re-throwing, so the turn's advanced state isn't lost on reload. The host-level error status (runtimeSlice 'error' via runtimeErrored) already existed. An error STATE inside the deterministic runtime was deliberately NOT added — an effect-execution error is a host concern (the runtime doesn't run effects), so modelling it in claw-core would mean a speculative MarkError command + contract surface with no consumer. Tested: runtimeHost.test.ts "flushes a snapshot when a fatal effect error halts the turn". -->
...
- [ ] Implement real model download queue state. <!-- NOT blocked, just deferred + low priority: the models slice already tracks per-model download state (downloads: Record<id, {status,progress}>) and the UI lists all in-flight downloads, but concurrent downloads aren't serialized/limited and the 'queued' status isn't used. "Real queue state" = enforce a max-concurrency queue + surface 'queued'. No consumer is pushing for it (downloads work; cancel works), so left for a dedicated pass. -->
...
- [x] Implement cancel. <!-- Buildable in our code (the "upstream-blocked" framing was stale): wllama 3.4.1 loadModelFromHF accepts DownloadOptions.signal (AbortSignal), and our IndexedDB fallback uses plain fetch. modelManager.download creates an AbortController per download (keyed in a Map), threads signal -> engine.download -> loadFromHF/runLoad -> loadModelFromHF({signal}) AND fetchGguf(url,{signal}); manager.cancel(modelId) aborts it. On abort the catch reports status 'cancelled' + audits model.download_cancelled (status 'cancelled') and does NOT rethrow (a cancel isn't a failure, so no error toast). The cache is abort-safe (blob only put after full download). ModelsScreen shows a Cancel button while status==='downloading'. -->
- [x] Implement pause only if truly supported; otherwise disable UI. <!-- NOT supported by wllama 3.4.1 (it exposes AbortSignal/cancel, not pause/resume) and Fetch has no native pause — a real pause would need hand-rolled resumable Range downloads. Per "otherwise disable UI", there is deliberately NO pause control (none was ever shown). Resolved as correctly-omitted. -->
...
- [x] Tests: <!-- all 8.2 test items covered (quota preflight, failed-download-not-cached, cancel updates state honestly, delete removes cache, loaded status requires real loaded model). -->
  ...
  - [x] cancel updates state honestly; <!-- wllama.test.ts "cancels an in-flight download and reports it honestly (not an error)": a download that only ends on abort is started, manager.cancel() aborts it, the promise resolves WITHOUT throwing, the slice shows status 'cancelled' (progress 0), and a model.download_cancelled audit (status 'cancelled') lands. -->
...
- [ ] Implement settings for:
  ...
  - [x] fallback provider; <!-- Real failover with a genuine consumer. appSettings getFallbackProviderId/setFallbackProviderId (key 'fallbackProviderId', default null). llmRunner: on a primary provider failure, if the error is reachability/transient (isFailoverEligible: cors|unreachable|unknown — NOT auth/model_not_found) and a fallback is configured, it retries ONCE on the fallback with the fallback's OWN SecretVault key (fail-closed: a locked/missing fallback key skips failover and surfaces the original error — never a silent unauthenticated call), audits provider.failover_used on success, and surfaces the fallback's error if it also fails. main.tsx wires resolveFallback (resolves the fallback profile+key, skips it when unset or equal to the active provider). SettingsScreen "Fallback provider" Select (None default + DEFAULT_PROVIDER_PROFILES). Tested: errors.test (isFailoverEligible), llmRunner.test (fails over on unreachable + audits; NEVER on auth; skips on locked fallback key -> original error; surfaces fallback's error when it also fails), appSettings.test (default none + round-trip), SettingsScreen.test (wiring). -->
  - [x] lock timeout; <!-- First setting wired END-TO-END: SettingsScreen lock-timeout Select now reads the durable value from app_settings on mount (getLockTimeoutMinutes) and writes changes back (setLockTimeoutMinutes); main.tsx restoreLockTimeout applies the persisted value to the SecretVault at boot; SecretVault.setLockTimeout(ms) re-arms the real auto-lock timer (the genuine consumer — secretVault.ts:243). Not a hollow control: it drives actual idle-locking. -->
        <!-- Theme, default provider, fallback provider, approval policy, and lock timeout are now wired end-to-end (read on mount, write on change, applied/dispatched live + at boot; approval policy is read by createToolEffectHandler, fallback provider by llmRunner). Remaining settings still decorative/local-only: backup, skill install policy, dev/demo/fallback mode (the last is env-gated via VITE_DEMO_MODE/appConfig, not runtime-mutable). -->
        <!-- The two umbrella boxes above (read-from / write-to IndexedDB) stay UNTICKED until more controls are wired — theme, default provider, fallback provider, approval policy, and lock-timeout read/write so far. -->
