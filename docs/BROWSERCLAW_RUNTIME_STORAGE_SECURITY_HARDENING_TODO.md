...
- [ ] Memory write approval -> memory persisted -> audit persisted. <!-- contract-blocked: a runtime-driven memory write is a storage_* effect routed through ctx.ports.storage in effectExecutor, which is the unwired Phase 5.1 storage handler (needs the claw-schema/WASM effect contract). The approval card (ApprovalCard) + audit_append effect exist, but the persist-on-approve path can't be exercised end-to-end until the storage port is wired. -->
