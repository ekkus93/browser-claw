...
- [ ] Append audit events for:
  ...
  - [ ] memory created/updated/deleted; <!-- PARTIAL: memory.created is now wired — the Remember tool audits it on a runtime-driven write (toolRunner.test asserts a success event with skillId). updated/deleted still unaudited (MemoriesScreen edits/deletes Dexie rows directly without an audit event). -->
