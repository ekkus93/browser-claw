...
<!-- FIX1 reconciliation 2026-06-28: Several items retroactively marked LIBRARY-ONLY or UNCHECKED after review found gaps between library tests and live app wiring. See docs/BROWSERCLAW_WORKSPACE_SCRIPTING_WEBRESEARCH_FIX1_TODO.md for the correction pass. -->
...
- [x] P0 Workspace write/delete approvals work. <!-- LIBRARY-ONLY per FIX1: runtime/workspaceRunner.test.ts — 'queues a mutating op for approval with the right kind' (workspace_write/workspace_delete); 'runs a read-only search directly without approval'; 'executes an approved op and resolves with the stat'; 'declines a rejected op and audits permission_denied'. All tested in isolation. Not wired into main.tsx yet — pending FIX1-B1/B2. -->
...
- [x] P0 Plan approvals/audits work. <!-- LIBRARY-ONLY per FIX1: runtime/planRunner.test.ts: 'queues a plan proposal for approval'; 'runs an approved plan and resolves the effect with its outputs' (plan_completed audit, ctx.fs.readText verified); 'declines a rejected plan' (plan_rejected audit). All tested in isolation with injected deps. registerRuntimeListeners + effect ports are not yet wired into main.tsx — pending FIX1-B1/B2. -->
...
- [x] P1 Search provider interface works with at least one configured provider or an explicit stub disabled in production. <!-- LIBRARY-ONLY per FIX1: webresearch/braveSearch.ts is a full SearchProvider library (search(q, opts) → SearchResult[]); isolated-tested in braveSearch.test.ts (15 tests, no live browser CORS). The live wired path (extension-backed Brave Search in the running app) is not assembled yet — pending FIX1-B1/G1/G2. Fail-closed behavior for missing provider is tested (service.test.ts). Key in SecretVault only. -->
...
- [ ] P0 Extension can read current tab. <!-- UNCHECKED per FIX1: extension/service-worker.js does NOT implement read_current_tab — the default: branch returns {ok:false, error:{kind:'unsupported'}}. BrowserClaw-side protocol wrapper (extension/pageReaderProvider.ts) exists and is tested, but the service-worker handler is absent. Implement in FIX1-A3/A4. -->
- [ ] P1 Extension can read approved search result URL. <!-- UNCHECKED per FIX1: extension/service-worker.js does NOT implement read_page — falls through to unsupported default. BrowserClaw-side protocol wrapper exists and is tested; service-worker handler is absent. Implement in FIX1-A3. -->
...
- [ ] P0 BrowserClaw shows extension missing/connected states honestly. <!-- UNCHECKED per FIX1: extension/service-worker.js get_status currently returns pageReadingAvailable:true, but read_page is NOT implemented — a hollow affordance. Must fix to pageReadingAvailable:false until FIX1-A3 lands. BrowserClaw UI side (WebResearchStatus.tsx, extensionRunner.ts) correctly surfaces missing/connected state, but is fed a dishonest pageReadingAvailable:true from the worker. -->
- [x] P1 Web research can search, read, and store result pages in workspace. <!-- LIBRARY-ONLY per FIX1: webresearch/storage.test.ts (H2) tests via mock search + mock reader (not a live extension call). The live extension-backed search path (Brave Search via extension service worker) is not wired yet. Implement live path in FIX1-B1/G1. -->
...
- [ ] P1 Chrome extension manual QA complete. <!-- UNCHECKED per FIX1: extension service-worker.js has a correctness defect (pageReadingAvailable:true when read_page is not implemented). 2/9 H3 items code-verified; 7/9 deferred manual. FIX1-A1 must fix the status defect; FIX1-A3/A4 implement real page reading; FIX1-K1/K2 add Dockerized E2E. Re-check this item after FIX1 Part A + K are complete. -->
