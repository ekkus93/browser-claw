...
describe('createLlmRequestHandler', () => {
  ...
  // FIX1-C3: plan / script / web block parsing in the LLM runner.
  describe('C3 — agent action block parsing', () => {
    const PLAN_BLOCK = `\`\`\`browserclaw-plan
${JSON.stringify({ type: 'browserclaw_plan', version: 1, title: 'T', reason: 'R', steps: [{ id: 's1', op: 'fs.readText', path: '/workspace/a.md' }] })}
\`\`\``;

    const SCRIPT_BLOCK = `\`\`\`browserclaw-script
${JSON.stringify({ type: 'browserclaw_script_request', version: 1, runtime: 'sandboxed_script', title: 'T', reason: 'R', code: 'return 1;', capabilities: {}, limits: { timeoutMs: 5000, maxOutputBytes: 1024, maxFileReads: 1, maxFileWrites: 1 } })}
\`\`\``;

    const WEB_BLOCK = `\`\`\`browserclaw-web
${JSON.stringify({ type: 'browserclaw_web_request', version: 1, op: 'readPage', url: 'https://example.com/page' })}
\`\`\``;

    const MALFORMED_PLAN_BLOCK = '```browserclaw-plan\n{ not json }\n```';

    async function setupConversation(id: string, userId: string) {
      await db.open();
      await db.messages.put({
        id: userId,
        conversationId: id,
    ...
    function providerWith(text: string): LlmProvider {
      return {
        id: 'mock',
    ...
    it('C3: valid plan block resolves effect with plan payload', async () => {
      ...
      expect(submit.mock.calls[0]?.[0]).toMatchObject({
        ...
        result: {
          ok: true,
          plan: expect.objectContaining({ type: 'browserclaw_plan' }),
        },
      });
    });
